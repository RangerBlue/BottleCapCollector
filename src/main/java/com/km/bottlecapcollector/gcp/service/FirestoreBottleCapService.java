package com.km.bottlecapcollector.gcp.service;
...
import com.km.bottlecapcollector.gcp.exception.FirestoreDocumentNotFoundException;
import com.km.bottlecapcollector.gcp.mapper.FirestoreBottleCapMapper;
import com.km.bottlecapcollector.gcp.repository.FirestoreBottleCapRepository;
import com.km.bottlecapcollector.gcp.storage.CloudStorageService;
...
public class FirestoreBottleCapService {
     ...
     * @param file the image file to process
     */
    private void processAndAttachImage(CollectionItem item, MultipartFile file) throws IOException {
        ...
        FirestoreVisionMetadata visionMetadata = visionApiService.analyzeImageFromFile(file);
    ...
    public CollectionItemResponse uploadImage(String collectionName, String id, MultipartFile file) throws IOException {
        ...
        // Upload new image
        StorageImage image = cloudStorageService.uploadImage(file, id);
        item.setImage(image);
        item.setUpdatedAt(Instant.now());
    ...
    private List<CheckCapResponse.SimilarCapDto> findSimilarItemsWithEmbeddings(
        ...
        for (CollectionItem item : candidates) {
            if (item.getId().equals(excludeItemId)) {
                continue;
            }
            ...
            if (uploadedEmbedding != null && uploadedEmbedding.getVector() != null &&
                ...
                similarityScore = HSBColorService.calculateHSBSimilarity(hsbColor, item.getImage().getHsbColor());
